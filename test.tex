...
\AddEverypageHook{\begin{tikzpicture}[remember picture,overlay]
		\fill[blue!50!green] (current page.north west) rectangle (current page text area.north -| current page.east);
		\node[color=white] at (current page header area) {\color[HTML]{ffffff}\fontfamily{Montserrat-TOsF}\selectfont\MakeTextUppercase\Huge\textbf{SIMON FISH}\\};
		\fill[blue!50!green] (current page.south west) rectangle (current page footer area.north -| current page.east);
		\node[color=white,yshift=-0.75cm] at (current page footer area) {\small\textit{simon.fish | si@mon.fish | 07852 406718 | 42 Tolethorpe Close, Oakham, Rutland LE15 6GF}};	
\end{tikzpicture}
}
